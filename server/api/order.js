...
async function findOrder(userId) {
  try {
    const order = await Order.findOne({
      /// find the one specific order that is in the cart and going to return it
      where: {
        userId,
...
async function findOrderLines(orderId) {
  ///finding the order lines given an order. so now you have a users order and want to find all the products in the order
  try {
...
async function findOneOrderLine(orderId, productId) {
  try {
    const product = await OrderLine.findOne({
      /// finding a specific orderline given an orderId and productId
      where: {
        orderId,
        productId,
      },
...
async function findProduct(productId) {
  /// we are finding the product based on the product id using the findBy primary key and returning the product
  try {
...
router.get('/', async function (req, res, next) {
  /// Getting the order and getting all the products in that order
  try {
    ...
    if (!order) {
      ///we say that if they don't have an active order
...
router.post('/', async function (req, res, next) {
  try {
    ...
    if (!order) {
      /// if they don't have an order then we are creating one
    ...
    if (!orderLine) {
      ///then we are finding the particular oderline and if there is not an orderline we are creating one
      ...
      await orderLine.update({
        /// here we are updating quantity and subtotal instead of creating a new orderline each time
